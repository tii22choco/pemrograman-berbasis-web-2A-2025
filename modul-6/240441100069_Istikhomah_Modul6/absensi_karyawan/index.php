...
$result = mysqli_query($conn, "SELECT * FROM karyawan_absensi GROUP BY nip");
?>
...
<head>
  <meta charset="UTF-8" />
  <title>Data Karyawan - Bloomie Florist</title>
  ...
  <div class="sidebar w-64 flex-shrink-0 text-white hidden md:block">
    <div class="h-full flex flex-col">
      ...
      <nav class="flex-1 px-4 space-y-2">
        <a href="index.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg bg-white/10 font-medium">
    ...
    <main class="flex-1 overflow-y-auto p-6 z-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div class="bg-white rounded-xl shadow-md p-6 flex items-center space-x-4 border-l-4 border-green-600">
          ...
          <div>
            <div class="text-gray-500 text-sm">Total Data Karyawan</div>
     ...
     <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-green-800">
          ...
          <table class="min-w-full divide-y divide-green-200">
            <thead class="bg-green-700">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Umur</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Jenis Kelamin</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Departmen</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Posisi</th>
            ...
            <tbody class="bg-white divide-y divide-green-200">
              ...
              <tr class="hover:bg-green-50 transition">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?= htmlspecialchars($row['nip']) ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= htmlspecialchars($row['nama']) ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= htmlspecialchars($row['umur']) ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= htmlspecialchars($row['jenis_kelamin']) ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= htmlspecialchars($row['departemen']) ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= htmlspecialchars($row['jabatan']) ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                  <div class="flex space-x-2">
                    <a href="edit.php?id=<?= $row['id'] ?>" class="text-green-700 hover:text-green-900">
                      <i class="fas fa-edit"></i>
                    </a>
                    <a href="hapus.php?id=<?= $row['id'] ?>" onclick="return confirm('Apakah kamu yakin ingin menghapus data karyawan?')" class="text-red-600 hover:text-red-900">
